...
for outFile in sys.argv[3:]:
	...
	if sys.argv[1] == "true":
		size_reduction = float(np.log2(float(l2_size) / l2_size_base))
		while (size_reduction > 0.0):
			ipc *= 0.85
			size_reduction =- 1.0

		assoc_reduction = float(np.log2(float(l2_assoc) / l2_assoc_base))
		while (assoc_reduction > 0.0):
			ipc *= 0.90
			assoc_reduction -= 1.0

	l1ConfigStr = '{}K.{}.{}B'.format(int(l2_size),int(l2_assoc),int(l2_bsize))
	print(l1ConfigStr)
...
if sys.argv[1] == "false":
	plt.savefig('L2_{}.png'.format(simname), bbox_inches='tight')
elif sys.argv[1] == "true":
	plt.savefig('L2_{}_reduced.png'.format(simname), bbox_inches='tight')
