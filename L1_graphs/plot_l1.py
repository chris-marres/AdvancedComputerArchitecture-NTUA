...
for outFile in sys.argv[3:]:
	...
	if sys.argv[1] == "true":
		size_reduction = float(np.log2(float(l1_size) / l1_size_base))
		while (size_reduction > 0.0):
			ipc *= 0.85
			size_reduction =- 1.0

		assoc_reduction = float(np.log2(float(l1_assoc) / l1_assoc_base))
...
if sys.argv[1] == "false":
	plt.savefig('L1_{}.png'.format(simname), bbox_inches='tight')
elif sys.argv[1] == "true":
	plt.savefig('L1_{}_reduced.png'.format(simname), bbox_inches='tight')
